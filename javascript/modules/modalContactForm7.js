...
document.addEventListener('DOMContentLoaded', () => {
	...
	const ModalForm = class {
		...
		bindFoundLinks = () => {
			const targetLinks = document.querySelectorAll(`[data-modalform_target="${this.modalID}"]`)
			...
			targetLinks.forEach(link => {
				...
				link.addEventListener('click', e => {
					...
					// Procesar el texto con plantillas de tags tipo {{title}}
					const processedData = this.replaceTemplateTags(formInputData, tags)

					// Si hay plantillas procesadas, usar el resultado procesado, sino usar el original
